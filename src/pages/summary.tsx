...
export default function SummaryPage() {
    ...
    const fetchSummary = async () => {
      try {
        ...
        const res = await fetch(
          `${process.env.REACT_APP_API_ENDPOINT}/user/summary/?account_id=${accountId}`,
        );
        if (!res.ok) {
  ...
  const handleScraping = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.REACT_APP_API_ENDPOINT}/web-scraping/?${keywords
  ...
  const handleDownloadPDF = () => {
    window.open(
      `${process.env.REACT_APP_API_ENDPOINT}/user/summary/pdf/?account_id=${accountId}`,
      '_blank',
    );
  };
